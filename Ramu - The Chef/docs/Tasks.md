...
**Private:** **v1.47.0** — deployed Sep 11, identity confirmed `offroadinggamedev@gmail.com`. v1.47.0 the dish-art race fix (`8d7802a`) · v1.46.0 Round J, the stag tuning pass (`8a0cfab`) · v1.45.0 Round I (`ed2ef78`). ✅ **All three verified from source and accepted** — [Implementation Handover Record](Implementation%20Handover%20Record.md). 🔒 **Review stays at v1.42.0 (Approved) and Public at v1.42.0 — untouched.** No `set-public`/`set-private`/`update-tag` has been run. 🔴 **v1.43.0's dish sprites have never reached public** — every live player still sees the Sep 4 insect art. Promotion is deliberately held until the level build lands, so marketing starts from that point ([LevelBlocks.md](LevelBlocks.md)). [GDD.md](GDD.md) §10.11c–e · [Specs.md](Specs.md) §8d
...
✅ **The Challenge Mode level design is CLOSED** — nine blocks, one belt, every parameter decided: [LevelBlocks.md](LevelBlocks.md). ✅ **And it is now losable** — Round I: a fully-maxed board loses at level **86**, `balanced` at 52, `fox-spam` at 49. 🔴 **But difficulty is carried entirely by stags**: every life lost in the 86-level maxed run comes from a 38–40 unit stag wave, while every boss wave at 42–71 units is a zero-leak shutout. The curve grows by HP inflation, not density — block 8 level 71 is *21 beetles at 44.1× HP* — and never approaches its own 120-unit ceiling, peaking at **80**.
...
⏸️ **Round queue, updated Sep 11:** ✅ **1· Balance — DONE** (Round I, `ed2ef78`) · 📤 **1b· Round J — the stag tuning pass**, in flight · 📤 **1c· The dish-art race** (see below), in flight · 📤 **2· Art generation** (nine backdrops, parallel) · **3· Visual** (tower reskin · backdrops · build sidebar · ghost slots · HUD rename · wave roster panel) · ~~4· Round B~~ — 🔴 **CUT Sep 11**, no room.
