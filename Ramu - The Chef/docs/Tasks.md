...
rename resubmit is untested until the user plays. **Round 11 returned Sep 18 ~00:10 IST → Private 1.83.0** (six playtest fixes + Ranks service board + daily period; both all-time boards unchanged). **Round 12 returned → 1.84.0** (Ranks B, fixes, near-you/delta) but ships a half-scale board (BOTTOM_BAND 2400). **In flight:** **Round 12b** → 1.85.0 (pixel-reserved HUD bands, laurel wired). **Queued:** R13 belt chevrons +
