...
rename resubmit is untested until the user plays. **Round 11 returned Sep 18 ~00:10 IST → Private 1.83.0** (six playtest fixes + Ranks service board + daily period; both all-time boards unchanged). **Round 12 returned → 1.84.0** (Ranks B, fixes, near-you/delta) but ships a half-scale board (BOTTOM_BAND 2400). **Round 13 returned → 1.87.0** (chevrons, i18n table English-only, popover submenu). **In flight:** **Round 14** → 1.88.0 (static scale, icon chips A, recipe shards + scroll cards, unlock sting, scroll/shard art). **Queued:** R13 belt chevrons +
