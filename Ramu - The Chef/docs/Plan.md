...
### 🛑 Sep 10 · KITCHEN MODE GATE — decided Sep 4

- [ ] **Is the belt playable end to end?** One recipe spawn-to-tray, no crash in ten runs, walkouts decrement, shift ends, end screen shows, menu round-trips. Procedural textures are fine — art is not part of this gate
- [ ] **If no: cut it.** Remove the `Kitchen (beta)` button, stop the work, and give Sep 10–19 to the return loop. Not paused — stopped, so the live entry carries nothing half-built into the final week. Item 52, [KitchenMode.md](KitchenMode.md) §5

### Sep 10–12 · Craft pass (CP7) — Editor's Pick, $300, independent of plays

**Trimmed from 8 h to 5 h** to absorb the level run.

- [-] ~~**Art Phase 2a — pre-rendered 3D sprites (GDD §11a).**~~ ➖ **Cut Sep 4, 22:05
      IST — items 12 and 51.** The art direction is **Props for the room, Essentials for
      the line**: two 2D packs at two scales, separated by depth. KayKit renders would be
      a *third* source at a fourth resolution and technique. **This frees the craft pass
      of a Blender render job the user would have done by hand**
- [ ] **Art Phase 2a′ — dress the room with Kitchen Props (replaces the above).** Place
      the recoloured isometric furniture as a background layer behind the belt: counter
      runs `KP1-78…83`, cabinets and fridges `KP1-84…91`, tables and chairs
      `KP1-96…114`, KP3 corners to turn a run. **Never scale a Props tile up to match a
      line sprite** — the honest size difference is the depth cue.
      [PropSpriteIndex.md](PropSpriteIndex.md) §5. Zero credits, art already owned
...
| 12 | ~~Render KayKit props to transparent PNG sprites~~ | User | — | ➖ **Cut Sep 4, 22:05 IST with item 51.** The art direction is two 2D packs at two scales, separated by depth. A third source rendered from 3D would be a fourth resolution and a fourth technique in one frame. Nothing is lost: this was scheduled work the user would have done by hand in Blender |
...
| 8 | Author the level run | **User** | Sep 10 | 🔄 **Re-scoped Sep 6**: **~38 levels across 5 nodes**, not 18 across 3 — each node is 1–2 grinding levels → 6 recipe levels → 1 boss. Dishes picked ([RecipeList.md](RecipeList.md) §7). ⬜ KitchenMode §7.2 recommends cutting the jam build to **Beverages + North Indian**, ~13 levels. Schema in [Specs.md](Specs.md) §6a — data only, no code |
...
| 43 | ✅ **Sample gains auditioned against a real playthrough** | User | — | ✅ **CLOSED Sep 5, 00:50 IST — confirmed by ear on v1.5.0, and the measured values needed no adjustment.** `lose` 0.5 · `upgrade` 0.35 · `wave-clear` 0.72. Original note: 🟢 `lose` 0.5 · `upgrade` 0.45 · `wave-clear` 0.5, set by peak-matching the synth they replaced (−3 dBFS samples vs 0.35/0.30/0.35 synth peaks). Arithmetic got them close; the last few dB are an ear. One-line constants at `src/audio/audio.ts` — the SAMPLES table. ✅ **Measured Sep 5, 00:35 IST — [Specs.md](Specs.md) §8a.9, `tools/qa_sfx.py`. Headroom is fine (+11.3 to +13.0 dB over every bed); the gains are not.** Peak-matching hid a **5.3 dB spread** in perceived loudness: matched on the loudest 300 ms, `upgrade` 0.45 → **0.35** (2.2 dB too loud) and `wave-clear` 0.50 → **0.72** (3.2 dB too quiet), with `lose` 0.50 as reference. `wave-clear` is the worst case twice over — quietest *and* its 28% low-mid sits under `service_low`'s dominant 60%, which is the bed it plays over. **Those are starting points, not answers — the ear settles the last 2 dB.** ⚠️ **Audition against `service_low` specifically** (Specs §8a.7b): the three BGM cues are trimmed to that cue as reference, so a gain fitted while a different cue is playing would be fitted to the wrong bed |
...
| 17 | ✅ **`textGen` surface closed** | Implementation agent | — | ✅ **CLOSED Sep 5, 15:10 IST in v1.7.0** (commit `0ff37e7`). `rundot/textGen.config.json` → `{ "disabled": true }`, file kept not deleted. Original note: 🟡 🟡 **Action specified Sep 4, 22:05 IST — one line.** Set `jam-entry/rundot/textGen.config.json` to `{"disabled": true}` and deploy. ⚠️ **The remedy this doc carried was wrong twice over.** RUN's bundled `rundot/docs/…/api/AI.md` §Configuration states the service is *default-bounded*: with **no** config file it still runs under platform defaults of ~\$500/game/day and ~\$10/user/day — so **deleting the file does not close the surface, it reopens it wider than a 500k cap** — and `rundot deploy` recreates the file the first time it detects the service in the bundle, which is how it appeared in the first place. `{"disabled": true}` makes every call fail `AI_POLICY_DENIED`. Two notes: the file is already tracked in git and the docs say to commit it; and **policy resolves from the published `public` tag**, so the edit is inert until a deploy publishes. The game never calls `textGen` — zero source references — so there is no behaviour to regress. See [Specs.md](Specs.md) §10 |
| 18 | Verify NCS + KayKit licences before either ships | User | Before Phase 2 art | ✅ **Closed Sep 4** — see items 10 and 11 |
| 19 | Credits/attribution screen — NCS track titles + the two itch.io pack creators | Implementation agent | Before BGM or rendered props ship | ⬜ **Ungated Sep 4** — the five SFX are CC0 and owe nothing, so this no longer blocks Phase 3. Still live for **NCS** (attribution is a standing condition of that licence). ⚠️ **KayKit is off this list from Sep 4, 22:05 IST** (item 12 cut) and is replaced by the two purchased itch.io packs — **toxiccolors** (*Kitchen Inventory Decoration*) and **hoshiixs** (*Kitchen Props*). Neither vendor publishes terms, so crediting them is a courtesy that costs nothing and covers the common unstated "attribution appreciated" — item 46 |
...
| 50 | ✅ **All three BGM cues chosen — no re-roll, no credits** | User | — | ✅ **CLOSED Sep 4, 22:35 IST.** **menu** = `bgm-menu-take1`, **service_low** = `bgm-service-low-take1` from run start, **service_high** = `bgm-service-high-take2` switched in with a crossfade **when walkouts remaining < 3** (of `startLives: 10`). The user proposed re-purposing take 2 rather than paying for a re-roll; the reasoning held and only the direction flipped. **Onset density settled it** — take 2 runs at **432 onsets/min**, the second-busiest file in the set, so it was never the slow track the 75 BPM estimate implied. The tempo figure was a triplet artifact, exactly as the original note warned. Assignment, per-cue gain trims and the trigger's three correctness properties are in [Specs.md](Specs.md) §8a.7 / 8a.7a / 8a.7b / 8a.7c. **113 credits saved.** Original note: 🔴 **Opened Sep 4, 21:03 IST from the take QA (Specs §8a.7).** Take 1 fades out 6 dB and cannot be looped. Take 2 is technically clean but measures **~75 BPM against a prompt that specifies 112 nine times** — and a high-intensity cue slower than the low-intensity one is the wrong direction for the cue that exists to raise tension. The tempo figure is a crude onset-autocorrelation estimate and 75 ≈ 112 × ⅔, so a triplet feel could be fooling it. **Needs a human listen before 113 credits are spent on a re-roll.** The other two cues are settled: `bgm-menu-take1` (cleanest numbers in the table) and `bgm-service-low-take1`. Blocked on the user |
| 51 | ✅ **Art direction — Props for the room, Essentials for the line** | User | — | ✅ **CLOSED Sep 4, 22:05 IST.** Original note: 🔒 opened 21:35 IST as *"Kitchen Props is a different art style, not just a different palette"*. The pack has been recoloured onto the Essentials palette and sliced: **284 items across four sheets**, numbered, named and catalogued in [PropSpriteIndex.md](PropSpriteIndex.md). The recolour is measured and it worked — b\* +2.3 → +12.3 against the target's +17.4, chroma 12.1 → 21.3 against 22.1, 57 colours in and 57 distinct out with every pixel mapped. **It is still not enough.** Essentials items are a median **94×124 px** of painterly, front-elevation art with antialiased edges; Props items are **32×32 px** of flat, binary-alpha, **isometric** pixel art — roughly 4× apart in resolution, with a different drawing technique and a different projection. Put a recoloured Props chair beside an Essentials handi and it reads as two games. **Three options, PropSpriteIndex §5:** (1) **Props for the room, Essentials for the line** — isometric furniture as background set-dressing behind the belt, at its own scale, with depth doing the separating; (2) **Props only**, rebuilding the line isometric — but the pack has **no ingredients at all**, so the whole PropList §4 interaction graph would be rewritten, with 15 days left and a live entry scoring; (3) **Essentials only**, Props unused. **DECIDED by the user Sep 4, 22:05 IST — option (1).** *"Props are for the room and Essentials are for the line — isometric furniture as background set-dressing behind the belt, at its own scale, with depth doing the separating."* ✅ **Closed.** Consequences: the belt, ingredients and stations stay Essentials, so the [PropList.md](PropList.md) §4 interaction graph is untouched; Props becomes a background layer that never needs to match scale because it never sits beside a line sprite; and **KayKit Phase 2a is cut** — a third art source at a fourth resolution is the same mistake the decision just avoided |
| 52 | 🔄 **Kitchen mode stop rule — REPLACED Sep 7 by the delivery gate, KitchenMode §6.8: all four cuisine nodes by Sep 19 or the live build carries the jam. §6.3's original gate was MET at round 8. Originally — Sep 10** | Planning agent + user | **Sep 10** | 🟠 **Set Sep 4, 23:10 IST, deliberately in advance.** If the belt is not **playable end to end** by Sep 10 it is **cut**, the menu button is removed, and the remaining days go to the return loop (S2) — which is what Total Unique Daily Plays actually rewards, and which is still unbuilt while D1 retention reads 0.0%. **"Playable end to end" is defined precisely** in [KitchenMode.md](KitchenMode.md) §5: one recipe spawn-to-tray without a crash ten runs running, walkouts decrement, the shift ends, the end screen shows, reachable from the menu and back. **Procedural textures are acceptable — art is explicitly not part of this gate.** The rule exists because writing it on Sep 4 costs nothing and making the same call on Sep 14, with six days sunk, is much harder to do well. 🔄 **NARROWED Sep 6, 01:00 IST — [KitchenMode.md](KitchenMode.md) §6.3.** The gate is now **the FTUE node only**: Chai spawn-to-tray, walkouts count, the shift ends, ten runs running. Two things changed — the belt moved to **its own branch behind a private build and the human gate**, so a half-finished belt can no longer reach the live entry, which is the damage the rule existed to prevent; and scope grew from one recipe to five nodes. ⚠️ **The justification quoted above is stale in one number**: D1 is measured at **2.2%**, not 0.0% (item 26) |
| 53 | ✅ **Music trigger fractioned** | Implementation agent | — | ✅ **CLOSED Sep 5, 15:10 IST in v1.7.0** (commit `0ff37e7`). Now `s.lives < CONFIG.economy.startLives * 0.3`; with `startLives: 10` that is arithmetically identical to the old literal `3`, so **nothing changed audibly today** — it is future-proofing for the belt's 5 walkouts. Original note: 🟠 🟠 **Amendment to a handover already in flight, Sep 4 23:10 IST.** The Phase 5 music handover triggers the high-intensity cue at `lives < 3`, calibrated as 20% of `startLives: 10`. Decision 7 gives the belt **5** walkouts, where a literal 3 is **60% remaining** — the cue would fire almost immediately and mean nothing. **Relay: use `lives < startLives * 0.3`.** Reads as 3 of 10 in tower defence and 1 of 5 in the belt, the same dramatic position in both. Everything else in that handover is mode-agnostic |
...
| 55 | 🟡 **Players cannot tell what to do after wave 1 — fix is LIVE, effect unmeasured** | Planning agent | **Measure Sep 6** | ✅ **LIVE Sep 5, 14:10:21 IST — v1.7.0 on all three tags** (commit `0ff37e7`): an eight-line hint, *"Tap a cook to upgrade"*, shown in the build phase on waves 2–4. **User-confirmed working.** Deliberately **not** a full FTUE — the user's call was to spend nothing expensive on a game view the belt is expected to replace, so FTUE proper moves into the belt (⚠️ **with the item 52 caveat: if the belt is cut Sep 10, this hint is the only onboarding that exists**). ➡️ **Still open because the fix is unproven, not unbuilt.** Baseline on v1.6.0: `wave_1_cleared`→`run_end` **40%**, `tower_upgraded` **22 of 70**. ⚠️ **The first clean read is Sep 6** — the scoring day rolls at 05:30 IST, so Sep 5 blends both builds and will understate the effect. **If neither number moves, the diagnosis was wrong**; that outcome gets recorded, not explained away. Specs §8a.11. Original note: 🔴 🔴 **Opened Sep 5, 12:56 IST from user-reported player feedback**, corroborated by three independent counts. Feedback: *entering the game mode, it is confusing to figure out what to do.* The funnel agrees and localises it — **placing a tower converts at 95%** (the build sheet is the only touchable thing on screen), but **`wave_1_cleared` → `run_end` converts at 40%** and **`tower_upgraded` reaches only 22 of the 70 players who placed a tower.** The 22 who upgrade and the 22 who finish a run are the same count. **Nothing teaches the second verb**, so from wave 2 the game stops responding to anything the player knows how to do, and they leave mid-run rather than losing. ➡️ **This is CP4** (*first-timer reaches the fun in under 30 s, unaided*), open since Sep 3 and now the measured reason the score is flat. **A daily reward cannot retrieve a player who never understood the game** — which is why item 31 ranks this above the S2 return loop and well above the Kitchen belt |
| 56 | 🔴 **92,047 credits expire Sep 15 — the only week we can ever buy traffic** | User + planning agent | **Sep 15** | 🔴 **Found Sep 9 2026 on the Finances → Credits dashboard.** Balance 129,247 splits into **92,047 event credits from code `BACK-TO-WORK`, expiring Sep 15**, a 25,000 starter grant showing a lapsed Aug 29 date, and 12,200 in quest rewards good to Dec 3–6. **Rollover: None.** After Sep 15 the account therefore holds ~12,200 — and RUN's marketing beta has a **\$50 campaign minimum** at **~1,000 credits per \$1**. Both post-cliff figures sit below the floor. ➡️ **Submit a campaign before Sep 15 or never run one for this jam.** `rundot marketing` is hidden behind `RUNDOT_BETA_FEATURES=1`; the CLI states *"RUN funds campaigns from your prepaid credits; a campaign spends only after RUN reviews and 'flights' it."* ❓ **The one unresolved question: does `submit` debit the credits, or does `flight`?** Evidence points at submit (*"prepaid budget is retained"*, *"the budget total is unchanged, so no credits were charged"*), which would convert expiring credits into ad budget that outlives the expiry — but that is **inference**, and review is a human step of unknown latency. **Ask RUN.** ✅ User confirmed Sep 9 that jam rules permit marketing; the only restriction is not entering two competitions at once. ➡️ **Consequence for art:** generation is **120 credits/image, exact** — spend it *freely* before Sep 15, since afterwards it competes for a 12,200 pool. ➡️ **Consequence for ads:** creative must be **brand-level**, because ads point at the game page, not a version — traffic lands on whatever build is live when it arrives, so submitting early does not mean advertising the tower game forever. ⚠️ Also on that dashboard: **Tipalti payouts are not set up**, which gates prize money; and a **runtime-AI line of 5,218 credits** is attributed to the live game this month — almost certainly pre-v1.7.0 spend, since `textGen.config.json` is `{"disabled": true}` and there are zero AI call sites, but the CLI does not expose that bucket, so **confirm it stopped before driving paid traffic** |
| 57 | ✅ **dobo_ui UI pack cleared for public deploy** | User | — | ✅ **CLOSED Sep 9 2026 — user ruling, "permission granted."** The last licence gate on the belt going public, and the only one item 46 did **not** cover: item 46 settled **toxiccolors** and **hoshiixs**, both purchased, while dobo_ui is the free **demo tier** of Cozy UI Pack and publishes no terms at all (`UI/developerNote_dobo_ui.txt` says only *"consider purchasing a complete version"*). It covers four shipping assets — `ui-billboard`, `ui-hotbar`, `ui-container` and round 25's two cards — which together are the **entire belt HUD**, so replacing them would have been an unscoped art round days from the deadline. [KitchenMode.md](KitchenMode.md) §6.7's *"not cleared for any public deploy"* line is **superseded** and must not be quoted as live again. ➡️ **Consequence:** nothing outside the implementation queue now gates a public belt — the remaining path is rounds only (bake → boss → save → analytics → menu + leaderboard). ✅ **The last open clause here is now CLOSED (Sep 9, round A2, `3e15c9d`):** `MainMenu.tsx`'s TEST MODE entry button was unconditional — and worse than the note suggested, it was the menu's ***primary*** CTA, so the public build's main button would have dropped every player into the unfinished belt. It is now gated behind `devMode.ts`'s `?test=1` latch, with Challenge Mode taking primary styling when it is hidden. `TestBelt.tsx`'s stale header (which also mis-cited KitchenMode §2.6 for a licence line §2.6 never contained) was corrected in the same round. ➡️ **With this closed, nothing gates a public deploy of v1.39.0** — promotion is now purely a decision, not a blocker |
| 58 | 🔴 **Live playtest feedback — RUN Discord session, Sep 9 2026** | User + planning agent | **Before any paid traffic** | 🔴 **The most valuable input of the project so far: real players, not inference.** Six findings. **(1)** FTUE must be impossible to lose and readable without words — the current *"Tap a locked station to unlock it"* text lands on a screen with too many poorly distributed elements; the player wants a visual cue (hover, pulse, size toggle), not a sentence. **(2)** Ingredient speed and hitbox window feel unforgiving in FTUE. **(3)** Players end a run *defeated, dejected and confused about what went wrong* — which is 1+2+5 compounding. **(4)** Music/SFX need **sliders** in pause, not toggles — ✅ **cheap: `save.ts` already stores `audio: {music, sfx}` as 0–1 floats and the bus takes continuous values, so this is UI-only.** **(5)** Win/lose unclear in **both** modes — 🔴 **verified worse than reported: `level.target` is never rendered anywhere in the belt**, so the player sees walkouts counting *down* and dishes counting *up with no stated ceiling* and cannot know a win condition exists; Challenge mode shows `Rush n/N` but labels lives only as an unexplained 🚪 emoji and never states what reaching 0 does. **(6)** No reason to return — achievements and rewards crucial. ➡️ **Answered by GDD §10.11** (scripted Challenge FTUE + Warrior achievements) and the belt FTUE work. ➡️ **Strategic consequence:** RUN's own UA playbook says fix FTUE *before* buying installs, and **every ad click before the belt goes public lands on Challenge mode** — so the live build's clarity is the highest-leverage work on the board, not the belt's. ➡️ **Credit timing (item 56):** submit before Sep 15 to bank the budget, **wait for `rundot credits` to actually drop**, *then* `marketing pause`, ship the FTUE fixes, and `resume`. 🛑 Never pause before the debit confirms — if credits are debited at flight rather than submit, pausing first means they expire unspent. ➕ Session also earned **10,000 credits** (balance 139,347); confirm on the Finances page whether that lot carries the Sep 15 expiry or a later one |
| 59 | 🔴 **The credit cliff is TWO cliffs, and the nearer one is Sep 13** | Finances page, read Sep 9 2026 | 🔴 **Sep 13 — 4 days** | 🔴 **Supersedes item 56's single-cliff read.** Balance **154,347**, and the source list sums to it exactly. **Two separate expiries:** **10,000 on Sep 13** (code `FEEDBACK05` — the RUN Discord playtest reward, which does **NOT** carry the Sep 15 date as item 58 assumed) and **92,047 on Sep 15** (code `BACK-TO-WORK`). ➡️ **Durable balance after Sep 15 = 52,300** — the 25,000 starter lot, quest rewards (12,200, Dec 3–6), the 15,000 grant (Dec 8), and 100/day. At ~1,000 credits = $1 that is **~$52: just clear of the $50 campaign minimum**, so a campaign remains *possible* after the cliff but only one, at the floor. Before it, **~$102** is available. Spending the expiring lots first is still clearly right — the margin is 2×, not infinite. ✅ **Two flags raised here on Sep 9 were both resolved by the user the same day, and both were mine to get wrong:** **(1)** *Starter credits +25,000, expires "Aug 29"* is **August 2029**, not Aug 29 2026 — the lot is live and durable, and reading a truncated date as a day-month was my error. **(2)** The *Runtime AI usage* line (5,218 credits, 3 features, 50 calls) is **our own art and music generation cost**, not player consumption — the panel's "consumed by players" wording misled me, and the grep that found **no AI calls in `src/`** was the correct signal, which I reported but then reasoned past. ➡️ **Consequence: ad traffic carries NO variable credit cost.** Campaign budget is the whole cost, which makes sizing a campaign a simple question of the expiring balance. See [Retro.md](Retro.md) lesson 83 |
| 60 | ✅ **The two campaign unknowns, answered by RUN admins** | RUN admins, Sep 9 2026 | — | ✅ **Both blockers on item 59 are closed, and they close in the direction that makes submitting safe.** **(1) Debit timing:** *"we take the credits when you submit but they get refunded if you cancel it or if we reject it, and we refund anything that doesn't get spent at the end."* ➡️ **Submitting BANKS an expiring lot** — the debit lands at submit, so credits captured before Sep 13/15 are converted to campaign budget even if the flight runs later. Downside risk is near zero: rejection and cancellation both refund. **(2) Approval lead time:** *"at most generally under an hour to push to public."* Confirmed empirically the same day — review 1.42.0 was approved and published to public within the session. 🔴 **Consequence: the pause dance in items 56/58 is now obsolete.** It existed to bank credits early while keeping traffic off a bad FTUE. **The FTUE is live as of v1.42.0**, so there is nothing to wait for — submit and let it run. ⚠️ **One residual unknown worth watching:** only credits actually *spent* are certainly rescued. Refunds of unspent budget arrive after the campaign ends, and whether they return to the original (expired) lot or a live one is unstated — so size the budget to what can plausibly be spent inside the jam window, not to the whole balance |
| 61 | ✅ **The 100-credit reconciliation gap — explained, not a missing lot** | Planning agent, Sep 9 2026 | — | The marketing agent flagged that the itemized lots sum to **154,247** against a confirmed balance of **154,347**, and reasonably inferred *"one un-itemized lot"* carrying its own expiry. ✅ **It is the Finances page's *Daily free credits +100 — Resets daily* line**, which its table omitted because it is a recurring allowance rather than a grant. With it, the sum is exact. ⚠️ **But the plan panel says Rollover: None**, so it does not accumulate — the precise durable figure is **52,200 in grants plus 100/day that never banks**, not a 52,300 lump. Every earlier "52,300" in this project quietly counted one day's allowance as if it persisted |
| 62 | 🔴 **Round H verified — every proof passes and the game still cannot be lost** | Planning agent, Sep 10 2026 | — | Round H (`004c4b8`, private v1.44.0) was checked from source against the human gate. ✅ **Every mechanical claim held**: `enemies.ts` absent from the diffstat, all pad coordinates byte-identical, `CONFIG.path` untouched, `SAVE_KEY` intact, `tsc` clean, and `npm run balance` reproduced exactly (fox-spam 66, balanced 69, miser 8, pad0-rush 76). The one flagged deviation — editing `sim/engine.ts`, outside the round's file table — was legitimate: Tasks 5 and 6 are engine rules, and the life-restore could not cap an upgraded player because lives take no meta modifier (`engine.ts:171`). 🔴 **But the round did not achieve its purpose, and three measured facts say why.** **(1) Splash has no distance falloff** — `engine.ts:493` deals FULL damage to every enemy in radius, so a maxed Tandoor's 170-unit radius covering ~6 units at level-80 spacing yields **~770 effective DPS from one tower**. Density is the defender's damage stat, which inverts the user's own fix of adding more enemies. **(2) The simulator has never modelled a real player** — `simulate.ts:141` calls `createEngine()` with no meta while the game calls `createEngine(getSave().meta)` (`towerScene.ts:190`); every balance number this project has produced was measured at 1.0× damage against a player at 1.5× damage, 1.4× fire rate, 1.2× range and +80 splash. **(3) Round H deleted HP scaling** — `hpMult`/`speedMult` are set on no authored level and no Overtime level, so `threat = Σ count × (hp × speed × walkouts) / pathLength` reduces to a weighted headcount and "strictly increasing threat" proves only "more enemies". ⚠️ **Measured consequence: 8 maxed Tandoors at full meta survive all 120 levels at 10/10 lives, and no enemy ever passes 14% of the path.** A single-target Grill board reaches 68% — the splash board is 5× better at holding the line than the single-target one, which is backwards. 🔥 **The framing error was the planning agent's**: the Round H threat formula was written with hp and speed as fixed per-unit constants, which made every proof satisfiable without making the game harder |
| 63 | ✅ **The Challenge Mode level design is closed — nine blocks, one belt** | User + planning agent, Sep 10 2026 | — | Settled across one session from four schematics (`references/Level Schematics/`) and recorded whole in **[LevelBlocks.md](LevelBlocks.md)**. 🔒 **One belt for all nine blocks**, so `CONFIG.path`, the slot array and `PATH_LENGTH` stay single constants and the design is a table rather than an architecture. Decided: nine themes; dish→archetype assignments for blocks 2–5 with one omission each; double sets alternating per spawn for 6–8; a **W1–W10 wave ladder identical in every block**; glow tiers (🔴 snail/hornet, 🟡 stag) instead of size variation; six of ten slots bonused; the HUD rename (**ESCAPES LEFT / CASH / WAVE + RUSH**, which closes the nomenclature brief owed since the Sep 10 playtest); a non-tower consumable coin sink; and Overtime as a repeating 10-level loop of 2 breathing / 5 amateur tough / 3 brutal. ⚠️ **Two numbers the design has to survive, both measured**: the fixed ladder makes block 8's W4 a snail-only wave of **291 units over 146 seconds**, and block 1's W7 a stag-only wave at **43 threat against a 21 budget**. ✅ **Both are fixed by the same decision** — restoring the per-level `hpMult`/`speedMult` item 62 found deleted: scale a snail up at level 74, and scale block 1's stag down to a **teaching boss** at a third of its HP that looks and costs like every other block's but cannot kill you. 🔴 **The approved board is also the strongest yet tested** — ten maxed Tandoors with six bonused slots hold every enemy inside **10%** of the path across 120 levels, against 12% for two bonuses and 14% for Round H's eight slots. Every step of the level design strengthened the defence; the balance round must size against this board, not the one the simulator assumes |
