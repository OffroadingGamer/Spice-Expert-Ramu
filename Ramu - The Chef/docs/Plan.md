...
| 59 | 🔴 **The credit cliff is TWO cliffs, and the nearer one is Sep 13** | Finances page, read Sep 9 2026 | 🔴 **Sep 13 — 4 days** | 🔴 **Supersedes item 56's single-cliff read.** Balance **154,347**, and the source list sums to it exactly. **Two separate expiries:** **10,000 on Sep 13** (code `FEEDBACK05` — the RUN Discord playtest reward, which does **NOT** carry the Sep 15 date as item 58 assumed) and **92,047 on Sep 15** (code `BACK-TO-WORK`). ➡️ **Durable balance after Sep 15 ≈ 27,300** — quest rewards (12,200, Dec 3–6), the 15,000 grant (Dec 8), and 100/day. At ~1,000 credits = $1 that is **~$27, below the $50 campaign minimum**, so a campaign is fundable **now and not later**. ⚠️ **Do not plan against 154,347 until one line is checked:** *Starter credits +25,000, expires **Aug 29*** is still listed and still inside the sum, though that date has passed — either the UI lists dead grants or the sweep has not run, and the **spendable** balance may be 25,000 lower. ⚠️ **Second, separate finding — player traffic burns credits.** *Runtime AI usage this month: Spice Expert: Ramu — **5,218 credits, 3 AI features, 50 calls*** (~104/call), described as *"credits consumed by players using AI features in your published games."* 🔴 **Our code makes no AI calls** — verified by grep across `src/`; every match is texture/wave generation or the engagement popups. So either RUN bills platform-level player AI to the game, or studio generation run against this game id is attributed here. **Confirm which before buying traffic**: if it scales with players, ad spend carries a variable credit cost on top of the budget, and ~27,300 durable credits is only ~260 calls |
