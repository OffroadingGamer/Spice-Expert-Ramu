...
> 🔄 **REORGANISED Sep 6 2026.** The sheets below are the slicer's raw output and
> are now an **archive, not a source of truth**. Every sprite lives in a typed folder —
> `Ingredient/` 27, `Container/` 34, `Cooking Oil/` 7, `Utensil/` 12, `Final Recipe/` 4,
> `props/` 44 — totalling 128, mapped in `_asset_map.csv`.
> `Untagged/` is dissolved. See [RecipeList.md](RecipeList.md) §8.5.
> ⚠️ **Do not pin an id to a `sheetN/` path.** [PropList.md](PropList.md) §4
> was re-pinned by content hash for exactly this reason.
